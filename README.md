...
| Aba | Serve para |
|---|---|
| `servicos` | Horário padrão de cada serviço (consulta, vacina, farmácia...) |
| `mudancas-horario` | O que muda **hoje/nesta semana** em relação ao horário padrão (fechamento pontual ou horário diferente) |
| `recados` | Avisos gerais, sem ligação com um serviço específico |
...
2. **Crie uma planilha do Google nova** para a unidade, com as 5 abas da tabela acima
   (`servicos`, `mudancas-horario`, `recados`, `equipe`, `faltas`) e compartilhe como "Qualquer pessoa
   com o link → Leitor".
3. **Edite só o [`config.js`](config.js)** da cópia nova:
   ...
   - Seções 3 a 6 — atualize os dados de reserva (`servicosReserva`, `avisosReserva`, `equipeReserva`)
